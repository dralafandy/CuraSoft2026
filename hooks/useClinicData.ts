...
export interface ClinicData {
    ...
    addPrescription: (prescription: Omit<Prescription, 'id' | 'userId' | 'createdAt' | 'updatedAt'>) => Promise<Prescription>;
...
export const useClinicData = (): ClinicData => {
    ...
    const fetchData = useCallback(async () => {
        ...
        if (treatmentDefsRes.data) {
            ...
            const formattedTreatmentDefinitions = treatmentDefsRes.data.map((def: any) => ({
                ...
                basePrice: Number(def.base_price) || 0, // base_price -> basePrice, ensure number
                doctorPercentage: Number(def.doctor_percentage) || 0, // doctor_percentage -> doctorPercentage, ensure number
                clinicPercentage: Number(def.clinic_percentage) || 0 // clinic_percentage -> clinicPercentage, ensure number
            }));
        ...
        if (treatmentRecordsRes.data) {
            ...
            const formattedTreatmentRecords = treatmentRecordsRes.data.map((record: any) => ({
                ...
                doctorShare: record.doctor_share, // doctor_share -> doctorShare
                clinicShare: record.clinic_share, // clinic_share -> clinicShare
                affectedTeeth: record.affected_teeth, // affected_teeth -> affectedTeeth
                totalTreatmentCost: record.total_treatment_cost || 0 // total_treatment_cost -> totalTreatmentCost
            }));
        ...
        if (patientAttachmentsRes.data) {
            ...
            const formattedAttachments = patientAttachmentsRes.data.map((attachment: any) => ({
                id: attachment.id,
                patientId: attachment.patient_id, // patient_id -> patientId
                filename: attachment.filename,
                originalFilename: attachment.original_filename, // original_filename -> originalFilename
                fileType: attachment.file_type, // file_type -> fileType
                fileSize: attachment.file_size, // file_size -> fileSize
                fileUrl: attachment.file_url, // file_url -> fileUrl
                thumbnailUrl: attachment.thumbnail_url, // thumbnail_url -> thumbnailUrl
                description: attachment.description,
                uploadedBy: attachment.uploaded_by, // uploaded_by -> uploadedBy
                createdAt: attachment.created_at, // created_at -> createdAt
                updatedAt: attachment.updated_at // updated_at -> updatedAt
            }));
    ...
    const addTreatmentRecord = async (patientId: string, record: Omit<TreatmentRecord, 'id' | 'patientId'>) => {
        ...
        if (error) {
            ...
            const newTreatmentRecords = newData.map((tr: any) => ({
                ...
                totalTreatmentCost: tr.total_treatment_cost || 0
            }));
    ...
    const updatePayment = async (payment: Payment) => {
        ...
        // Map Payment object to snake_case for Supabase
        const { id, patientId, date, amount, method, notes } = payment;
        const supabaseData = {
            patient_id: patientId,
            date,
            amount,
    ...
    const addSupplier = async (s: Omit<Supplier, 'id'>) => {
        ...
        const supplierData = {
            name: s.name,
            contact_person: s.contactPerson || null, // contactPerson -> contact_person
            phone: s.phone || null,
            email: s.email || null,
            type: s.type,
            user_id: user.id
