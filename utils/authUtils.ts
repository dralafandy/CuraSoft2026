...
export const authenticateInternalUser = async (
  ...
  try {
    ...
    // For internal users, we use a simple password check for demo
    // In production, implement proper password hashing
    const isValidPassword = password === '123' || validatePassword(password).isValid;
    console.log('AuthUtils: Password validation result:', isValidPassword);
    ...
    const { data: newUser, error: insertError } = await supabase
      ...
      .insert({
        ...
        permissions: role === 'ADMIN'
          ? ['view_dashboard', 'manage_users', 'view_patients', 'edit_patients', 'view_scheduler', 'edit_scheduler', 'view_reports', 'view_finance', 'edit_finance']
          : ['view_dashboard', 'view_patients', 'view_scheduler', 'view_reports']
      })
      .select()
      .single();
